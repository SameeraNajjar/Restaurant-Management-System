...
public class UserManagementController extends AbsController implements Initializable {
    ...
    private void deleteUser(Users user) {
        try {
            userDOA.delete(user.getId()); // Assuming userDOA has a delete method
            usersTable.getItems().remove(user); // Remove the user from the table

            // Show the popup after successfully deleting the user
